...
if __name__=="__main__":
    ...
    if img_file is not None:
        ...
        st.image("LBP_SVC_Results.png","testing result")

        features_lbp = local_binary_pattern(img,P=8, R=1, method="uniform").flatten()

        svc_lbp = pickle.load(open("LBP_SVC.pkl","rb"))
        ...
        st.image("HOG_with_NN.png","testing result")

        features_tensor = torch.tensor(features,dtype=torch.float32)

        nnet = NueralNetwork()
        nnet.load_state_dict(torch.load("NueralNetwork",weights_only=True))
        nnet.eval()
        ...
        st.subheader("CNN")
        st.image("CNN_results.png","test results")

        mpcnn = MaskPredictorCNN(1,32,1,128)
        nnet.load_state_dict(torch.load("CNN",weights_only=True))
